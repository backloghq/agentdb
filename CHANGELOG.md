...
- **`CollectionMetrics.writeMode`, `bm25DocCount`, `bm25MergePending`** — three new fields on the snapshot returned by `col.metrics()`. `writeMode` reflects the mode the collection was opened with (`"immediate"` / `"group"` / `"async"`; default `"immediate"`). `bm25DocCount` is the total indexed-and-flushed document count from `TermLog.docCount()` (null when text search is disabled). `bm25MergePending` is `true` when there is more than one BM25 segment (i.e. a compaction pass would consolidate them), `false` when fully merged, `null` when text search is disabled.
