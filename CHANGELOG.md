...
- **BM25 scoring parity regression tests added (phase 4)** — two new tests in `hybrid-search.test.ts` ("bm25Search — BM25 scoring parity") verify that `bm25Search` scores match a hand-derived reference implementation of the termlog BM25 formula (idf × tf × (k1+1) / (tf + norm)) to within 1e-9. Corpus: 3 docs, query "rust", single term, df=2. Tests run with default k1=1.2/b=0.75 and tuned k1=2/b=0.3 (via `defineSchema({ bm25: {...} })`).
