...
- **Task 313 — HNSW graph persistence via `hnsw/graph.bin` sidecar** — disk-mode collections now persist the HNSW graph topology to `<collectionDir>/hnsw/graph.bin` on `close()`, eliminating the O(N × M × efConstruction) rebuild on every reopen. Binary format: magic `0x484E5347` ("HNSG"), format-version u32, M u32, dims u32, maxLayer u32, length-prefixed entry-point id, per-node rows (id, topLayer, per-layer neighbor-id lists). Vectors are NOT stored in the sidecar — they are hydrated O(N) from the quantized `_embedding` fields already in Parquet/JSONL on load. Atomic write: `graph.bin.new` → `graph.bin.old` → `graph.bin` (rename dance). Crash recovery on open: orphan `.new` deleted, orphan `.old` (canon absent) promoted to canon, both present means leftover `.old` is removed. Fallback to `rebuildHnswFromDisk` on absent file, magic/version/M/dims mismatch, or nodeCount discrepancy after hydration (catches crashes that truncated the file or lost records). `HnswIndex.toBuffer()`, `HnswIndex.fromBuffer(buf, opts)`, and `HnswIndex.hydrateVector(id, vec)` are the serialization API. Consistency fix: `embedUnembedded` and `reembedAll` now add the dequantized form of each stored embedding to the HNSW index (matching what `loadHnswFromDisk` hydrates), ensuring search results are bit-for-bit identical across close/reopen cycles. 7 new tests in `hnsw-persistence.test.ts`: round-trip (20 vectors, results identical on reopen), large round-trip (200 vectors, nodeCount preserved), crash-recovery (.old-only restores canon), M-mismatch fallback (rebuilds cleanly), missing-sidecar fallback (rebuilds cleanly), crash-mid-write (.new-only cleaned and rebuild), determinism with seed (toBuffer/fromBuffer round-trip, repeated close/reopen identical ranking).
...
- **Task 306 — S3 `rebuildTextIndex` close-interlock gap** — `_rebuildAbortCtrl` and `_rebuildSettled` were only allocated in the FS branch, so `close()` could not signal or wait for an in-flight S3 rebuild. Both are now hoisted to before the S3/FS branch split, allocated synchronously before the first `await`, guaranteeing that a `close()` called immediately after `rebuildTextIndex()` always sees the controller set. The S3 branch's `finally` now clears `_rebuildAbortCtrl`/`_rebuildSettled` and calls `settleRebuild()` (matching the FS branch). Two new abort checkpoints added to the S3 path: a pre-wipe check (fires immediately on pre-aborted signals, before any destructive S3 operation) and a pre-loop check (fires after `TermLog.open` for zero-record collections and the close-during-setup window). `close()` JSDoc updated to say "FS or S3 mode" instead of "FS-mode". 2 new tests in `progress-callbacks.test.ts` using a gated `FsBackend`-backed mock: (a) pre-abort rejects with `AbortError`; (b) close()-during-S3-rebuild: interlock settles so `close()` completes and rebuild rejects. Test count: 1475 → 1477.

- **Task 305 — SubscriptionManager pin-while-subscribed** — LRU eviction in `AgentDB.evictLru()` now skips collections that have active change subscribers, preventing silent subscription breakage and stale closed-Collection retention. `AgentDB.pinForSubscription(name)` / `unpinForSubscription(name)` track a per-collection subscriber count; `SubscriptionManager.subscribe()` calls pin for each new subscriber and all three exit paths (`unsubscribe`, `removeSession`, `destroy`) call unpin symmetrically. `evictLru` scans the LRU list for the oldest unpinned entry and returns `false` when all candidates are pinned; the `_openCollection` eviction loop breaks on `false` (allows opening over limit) rather than spinning or evicting a subscribed collection. `AgentDB.close()` clears the pins map. 4 new tests in `subscriptions.test.ts`: pinned collection survives LRU eviction; unpinned collection is evicted; subscribe/unsubscribe/resubscribe/unsubscribe round-trip restores evictability; multiple subscribers require all to unsubscribe before eviction is allowed. Test count: 1471 → 1475.

- **Task 304 — `Collection.close()` listener and watch-interval leaks** — `close()` now calls `this.unwatch()` before closing the opslog store (stops the WAL polling interval) and `this.emitter.removeAllListeners()` afterwards (releases all subscriber closures). Previously, long-lived processes that repeatedly opened and closed collections accumulated EventEmitter listeners and setInterval handles. 2 new tests in `collection.test.ts`: listener count is zero after close; watch callbacks stop firing after close. Test count: 1469 → 1471.
...
- **Lazy auto-init (task 300)** — all async public methods (`collection`, `dropCollection`, `purgeCollection`, `listCollections`, `persistSchema`, `loadPersistedSchema`, `deletePersistedSchema`, `loadSchemasFromFiles`, `stats`, `rebuildTextIndex`, `export`, `import`) now call `_ensureInit()` internally before proceeding. A caller who skips `await db.init()` and goes straight to `await db.collection(...)` gets a working instance. `init()` is idempotent — subsequent calls return immediately; concurrent first callers share one init promise so `_doInit` runs exactly once. A failed init does not cache the rejection, allowing retry after transient failures (e.g. fixed directory permissions). 4 new tests: `AgentDB.open()` factory, lazy auto-init without explicit `init()`, `init()` idempotency, concurrent-first-call no-double-init race. Test count: 1486 → 1490.
...
- **R8/4 test gap bundle — 6 new tests across three files** (test count: 1451 → 1484):
  ...
  - **(c)** Close-during-swap (`progress-callbacks.test.ts`): skipped with comment — no `phase:'swap'` marker in `ProgressEvent`; covered by `rebuild-atomicity.test.ts`.
  - **(d)** Rollback-of-rollback (`rebuild-atomicity.test.ts`): new `_interceptRollbackRename` flag added to `vi.mock("node:fs/promises")`; arm both `_interceptStep2` and `_interceptRollbackRename`; assert ENOTEMPTY propagates, `console.error` fires with collection name and "text index unavailable", and `text/` is absent after the double failure.
  - **(e)** Crash recovery B non-destructive (`rebuild-atomicity.test.ts`): insert N old-beacon records + first rebuild, then M new-beacon-unique records + second rebuild; create stale empty `text.old/`; reopen; assert stale backup removed, `text/` intact, and `bm25Search("new-beacon-unique")` returns ≥ M results — proves the current index is not overwritten by the stale backup.
  - **(f)** Vendor-key body shape + constructor-key precedence (`embeddings-vendor-forwarding.test.ts`): assert Voyage sends `input: [...]` and Cohere sends `texts: [...]` in the request body; assert that passing an explicit `apiKey` to the constructor sends the canonical key even when `VOYAGE_API_KEY` / `COHERE_API_KEY` env vars contain a different fallback.

- **`rebuildTextIndex` rollback reopen failure now logs a named `console.error`** — when the rollback path successfully renames `text.old/→text/` but `TermLog.open` on that restored directory also fails (e.g. disk full mid-rollback, permissions changed), the silent `.catch(() => null)` is replaced with a diagnostic: `agentdb [<name>]: rollback reopen failed after rebuildTextIndex swap error — text index unavailable until next rebuild: <err.message>`. The original swap error is still rethrown so the caller sees the root cause; the log gives operators a named signal that the text index is gone rather than a silent null. 1 new test (`rebuild-atomicity.test.ts`): spy on both `vi.mock(node:fs/promises)` step-2 and `vi.spyOn(TermLog, 'open')` to inject both failures simultaneously; assert ENOTEMPTY propagates and `console.error` message contains the collection name and "text index unavailable". Test count: 1450 → 1451.
