...
- **Algorithm R reservoir sampling in `db_infer_schema`** — replaces window-based (contiguous offset) sampling with Vitter's Algorithm R. Each record in the collection has an equal probability of appearing in the sample, regardless of collection order. Iterates the full collection in 5000-record chunks; sampling note updated to say "Algorithm R reservoir sampling". Uniformity distribution test (50 runs, split-population check) added.
