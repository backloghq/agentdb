...
describe.skipIf(!process.env.BENCH)("BM25 + hybrid search benchmarks", { timeout: 600_000 }, () => {
  ...
  // 1. Disk-mode reopen with v2 stats vs v1 baseline
  ...
  // 2. Hybrid vs arms — relative latency
  ...
  // 3. RRF fusion overhead — two 1K and two 10K lists
  ...
  // 4. Imbalanced RRF — small list must still contribute to top-10
  ...
  // 5. Concurrent query/write — no errors, p99 bounded
  ...
  // 6. rebuildHnswFromDisk cold-open heap delta (skipCache flag)
  ...
  // 7. readJsonlStream cold-open heap delta — streaming vs Map materialisation
