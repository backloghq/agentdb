/**
 * R7/3a — Atomic rename failure modes and crash recovery for rebuildTextIndex.
 *
 * Uses vi.mock on node:fs/promises to inject a step-2 rename failure, then verifies
 * that the rollback path restores the original text/ index so the collection remains
 * queryable.  Also tests both crash-recovery branches that open() must handle:
 *   A) text.old/ present, text/ absent  → restore text.old/ → text/
 *   B) text.old/ and text/ both present → delete stale backup
 */
