...
describe("write modes", () => {
  ...
  describe("group commit", () => {
    it("inserts are buffered and persist after close/reopen", async () => {
      ...
      expect(col.findOne("g1")?.value).toBe("alpha");
      await db.close();
      ...
      expect(col2.count()).toBe(3);
      expect(col2.findOne("g1")?.value).toBe("alpha");
      expect(col2.findOne("g2")?.value).toBe("beta");
      expect(col2.findOne("g3")?.value).toBe("gamma");
      await db2.close();
    });
    ...
    it("supports updates and deletes with group commit", async () => {
      ...
      expect(col.findOne("u1")?.value).toBe("updated");
      expect(col.findOne("u2")).toBeUndefined();
      await db.close();
      ...
      await db2.init();
      const col2 = await db2.collection("items");
      expect(col2.findOne("u1")?.value).toBe("updated");
      expect(col2.findOne("u2")).toBeUndefined();
      await db2.close();
    });
    ...
    it("respects custom groupCommitSize and groupCommitMs", async () => {
      ...
      await db2.init();
      const col2 = await db2.collection("items");
      expect(col2.count()).toBe(10);
      for (let i = 0; i < 10; i++) {
        expect(col2.findOne(`c${i}`)?.idx).toBe(i);
      }
      await db2.close();
    });
  });
  ...
  describe("async mode", () => {
    it("inserts return fast and persist after close/reopen", async () => {
      ...
      expect(col.findOne("a1")?.value).toBe("one");
      await db.close();
      ...
      expect(col2.findOne("a1")?.value).toBe("one");
      expect(col2.findOne("a2")?.value).toBe("two");
      await db2.close();
    });
    ...
    it("supports batch insert in async mode", async () => {
      ...
      await db2.init();
      const col2 = await db2.collection("items");
      expect(col2.count()).toBe(20);
      expect(col2.findOne("b0")?.idx).toBe(0);
      expect(col2.findOne("b19")?.idx).toBe(19);
      await db2.close();
    });
  });
  ...
  describe("multi-writer fallback", () => {
    it("group mode with agentId still persists data", async () => {
      ...
      const col2 = await db2.collection("items");
      expect(col2.count()).toBe(2);
      expect(col2.findOne("m1")?.value).toBe("multi-group");
      await db2.close();
    });
    ...
    it("async mode with agentId still persists data", async () => {
      ...
      const col2 = await db2.collection("items");
      expect(col2.count()).toBe(2);
      expect(col2.findOne("ma1")?.value).toBe("multi-async");
      await db2.close();
    });
  });
  ...
  describe("immediate mode (default)", () => {
    it("works as baseline comparison", async () => {
      ...
      expect(col2.findOne("i1")?.value).toBe("immediate");
      await db2.close();
    });
  });
});
