...
- **Config knobs:** `embeddingBatchSize`, `diskConcurrency`, `cacheSize`, `rowGroupSize`, `maxFindLimit`, `maxIndexCardinality` — all follow AgentDB (db-wide default) + CollectionOptions (per-collection override) shape; `maxFindLimit` (default 10_000) caps `find()` results and emits `console.warn` on truncation; `maxIndexCardinality` (default 1_000) controls disk B-tree index use and emits `console.warn` once per field when exceeded
