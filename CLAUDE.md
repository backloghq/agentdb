...
**Status: v2.2.1 — v2.2.0 plus three operator-experience patches: (1) MCP HTTP CORS allows the spec-required `MCP-Protocol-Version` request header and exposes `Mcp-Session-Id` for browser fetch clients; (2) `db_archive_list` returns `Array<{ name, recordCount }>` (admin/operator views), with `details:false` for fast names-only mode; (3) `db.import()` / `db_import` returns structured `ImportResult` (`{collections, records, inserted, overwritten, skipped, errors[]}`) and the MCP tool emits `notifications/progress` when a `progressToken` is supplied. Known issue: HNSW `graph.bin` sidecar is FS-only — S3 deployments rebuild graphs from vectors on container restart; S3-native sidecar deferred to v2.3.**
...
Major capabilities:
- **Search:** `bm25Search` (BM25 via `@backloghq/termlog`, k1/b tunable, `searchable:true` per field, segment-based LSM, S3-aware), `semanticSearch` (HNSW, 6 providers, Int8 quantization), `hybridSearch` (RRF fusion, per-arm failure degrades gracefully), `searchByVector`; query planner auto-consults bloom filters for equality predicates — `definitely-not-present` short-circuits to empty before scan; B-tree/composite/array take precedence
- **Disk mode:** JSONL for point lookups + Parquet for column scans; LRU cache; async `entries()` iterator; `appendEmbeddings` writes embedding batches durably to JSONL (bypasses eviction race); `embedUnembedded` single-pass disk scan (halves S3 I/O); `reembedAll` for v1.3→v1.4 migration (`_id` was incorrectly included in embedding text) with mid-flight `compactInPlace()` every 8 JSONL files (bounds index rewrite cost for large runs); `rebuildHnswFromDisk` on reopen; HNSW graph persisted to `hnsw/graph.bin` sidecar on `close()` — atomic rename dance, crash recovery on open, fallback to rebuild on mismatch/corruption; **S3 limitation**: `hnsw/graph.bin` is written to local FS (not S3) — in S3 deployments across container restarts, HNSW always rebuilds via `rebuildHnswFromDisk`; S3-native sidecar tracked as v2.3 task (491a1ad0); composite and bloom indexes persisted to `indexes/composite-*.json` / `indexes/bloom-*.json` on `close()` — loaded O(1) from JSON on reopen (v2.1.1 O(N) disk-scan fallback for absent/mismatched files); composite+bloom persist works over S3 backend (verified by task 320 test 4)
...
- **Config knobs:** `embeddingBatchSize`, `diskConcurrency`, `cacheSize`, `rowGroupSize`, `maxFindLimit`, `maxIndexCardinality`, `filterCacheSize`, `mergeParquetThreshold`, `mergeJsonlThreshold`, `hnsw` — all follow AgentDB (db-wide default) + CollectionOptions (per-collection override) shape; `maxFindLimit` (default 10_000) caps `find()` results and emits `console.warn` on truncation; `maxIndexCardinality` (default 1_000) controls disk B-tree index use and emits `console.warn` once per field when exceeded; `filterCacheSize` (default 64) is a per-collection compiled-filter LRU cache; `mergeParquetThreshold`/`mergeJsonlThreshold` (defaults 10/8) control compaction triggers; `hnsw: { M?, efConstruction?, efSearch?, maxLevel?, seed?, persistEvery? }` (defaults 16/200/50/derived/unset/unset) configures HNSW index parameters; `seed` enables deterministic layer assignment via mulberry32 PRNG; `persistEvery` (default unset = close-only) fires an async full-graph flush to `hnsw/graph.bin` every N user-facing `add()` calls — bounds crash exposure to at most `persistEvery` un-persisted records at cost of write amplification (each flush rewrites the full graph)
