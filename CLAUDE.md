...
- **Config knobs:** `embeddingBatchSize`, `diskConcurrency`, `cacheSize`, `rowGroupSize`, `maxFindLimit` — all follow AgentDB (db-wide default) + CollectionOptions (per-collection override) shape; `maxFindLimit` (default 10_000) caps `find()` results and emits `console.warn` on truncation
