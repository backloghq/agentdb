...
- **Config knobs:** `embeddingBatchSize`, `diskConcurrency`, `cacheSize`, `rowGroupSize`, `maxFindLimit`, `maxIndexCardinality`, `filterCacheSize`, `mergeThreshold`, `mergeJsonlThreshold`, `hnsw` — all follow AgentDB (db-wide default) + CollectionOptions (per-collection override) shape; `maxFindLimit` (default 10_000) caps `find()` results and emits `console.warn` on truncation; `maxIndexCardinality` (default 1_000) controls disk B-tree index use and emits `console.warn` once per field when exceeded; `filterCacheSize` (default 64) is a per-collection compiled-filter LRU cache; `mergeThreshold`/`mergeJsonlThreshold` (defaults 10/8) control compaction triggers; `hnsw: { M?, efConstruction?, efSearch?, maxLevel? }` (defaults 16/200/50/derived) configures HNSW index parameters for approximate nearest neighbor search
